...
{
    public partial class UC_Hqs : UserControl, IFerramentas
    {
        public UC_Hqs()
        {
            InitializeComponent();
        ...
        public void AbrirFicha()
        {
            try
            {
                ...
                MySqlCommand comando = new MySqlCommand("SELECT ID, Titulo, Subtitulo, Colecao, Autor, Volume, Unico, Editora, Genero, Edicao, Proprietario, Observacoes FROM hqs WHERE Id = ?", conexao);
        ...
        public void SalvarFicha()
        {
            ...
            try
            {
                ...
                MySqlCommand comando = new MySqlCommand("INSERT INTO hqs (ID, Titulo, Subtitulo, Colecao, Autor, Volume, Unico, Editora, Genero, Edicao, Proprietario, Observacoes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", conexao);
                comando.Parameters.Add("@ID", MySqlDbType.Int64, 15).Value = Txt_Id.Text;
        ...
        public void AlterarFicha()
        {
            ...
            try
            {
                ...
                MySqlCommand comando = new MySqlCommand("UPDATE hqs SET Titulo = ?, Subtitulo = ?, Colecao = ?, Autor = ?, Volume = ?, Unico = ?, Editora = ?, Genero = ?, Edicao = ?, Proprietario = ?, Observacoes = ? WHERE ID = ?", conexao);
                comando.Parameters.Clear();
                comando.Parameters.Add("@Titulo", MySqlDbType.VarChar, 75).Value = Txt_Titulo.Text;
                comando.Parameters.Add("@Subtitulo", MySqlDbType.VarChar, 75).Value = Txt_Subtitulo.Text;
                comando.Parameters.Add("@Colecao", MySqlDbType.VarChar, 75).Value = Txt_Colecao.Text;
        ...
        public void ApagarFicha()
        {
            try
            {
                ...
                MySqlCommand comando = new MySqlCommand("DELETE FROM hqs WHERE ID = ?", conexao);
                comando.Parameters.Clear();
                comando.Parameters.Add("@ID", MySqlDbType.Int64, 15).Value = Txt_Id.Text;
                comando.ExecuteNonQuery();
        ...
        public void BuscarFichas()
        {
            try
            {
                ...
                MySqlCommand comando = new MySqlCommand("SELECT ID, Titulo, Subtitulo, Autor FROM hqs WHERE ID LIKE ? OR Titulo LIKE ? OR Subtitulo LIKE ? OR Colecao LIKE ? OR Autor LIKE ? OR Volume LIKE ? OR Unico LIKE ? OR Editora LIKE ? OR Genero LIKE ? OR Edicao LIKE ? OR Proprietario LIKE ? OR Observacoes LIKE ?", conexao);
                comando.Parameters.Clear();
